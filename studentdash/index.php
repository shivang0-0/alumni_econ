...
">
                        ...
                        <table class="table no-wrap">
                            ...
                            <tbody>';
                                ...
                                else if ($_POST['sel'] == "Show recent opportunities") {
                                    $servername = "localhost";
                                    $username = "root";
                                    $password = "";
                                    $dbname = "project";
                                    ...
                                    $sql = "SELECT A.cname,B.job_role,B.for_year,C.salary,C.job_description,C.job_location FROM company_details A,company_opportunity B,job_details C WHERE A.cid=B.cid AND B.job_role=C.job_role";
                                    $result = mysqli_query($conn, $sql);
                                    echo '<div class="table-responsive">
                        <table class="table no-wrap">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-top-0">Job Role</th>
                                    <th class="border-top-0">For Year</th>
                                    <th class="border-top-0">Salary</th>
                                    <th class="border-top-0">Job Description</th>
                                    <th class="border-top-0">Job Location</th>
                                </tr>
                            </thead>
                            <tbody>';
                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_array($result)) {
                                            echo '<tr><td>' . $row["cname"] . '</td><td>' . $row["job_role"] . '</td><td>' . $row["for_year"] . '</td><td>' . $row["salary"] . '</td><td>' .$row["job_description"] .'</td><td>'. $row["job_location"] .'</td></tr>';
                                        }
                                    } else {
                                        echo "0 results";
                                    }
